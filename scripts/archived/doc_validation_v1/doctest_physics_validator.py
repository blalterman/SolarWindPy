...
class PhysicsDocTestRunner(doctest.DocTestRunner):
    ...
    def _validate_source_patterns(self, example):
        ...
        if any(term in source for term in ['w_thermal', 'thermal_speed', 'w_par', 'w_per']):
    ...
    def _check_thermal_speed_source(self, source: str, example):
        ...
        hardcoded_pattern = r'w_thermal\s*=\s*(?:np\.random\.normal\()?([0-9]+\.?[0-9]*)'
...
def run_enhanced_doctests(module_path: str, 
                         verbose: bool = False,
                         physics_validation: bool = True) -> Dict[str, Any]:
...
def run_doctests_on_file(file_path: Path,
                        verbose: bool = False,
                        physics_validation: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        results['success'] = (results['tests_failed'] == 0 and 
                            len(results['physics_violations']) == 0)
...
def run_doctests_on_directory(directory: Path,
                             verbose: bool = False,
                             physics_validation: bool = True) -> Dict[str, Any]:
    ...
    results['summary'] = {
        'success_rate': (
            ...
            max(results['total_tests_attempted'], 1) * 100
        ),
        'physics_compliance_rate': (
            (results['total_examples_run'] - results['total_physics_violations']) /
            max(results['total_examples_run'], 1) * 100
        ),
        'files_with_violations': sum(
            1 for fr in results['file_results'] 
            if len(fr['physics_violations']) > 0
        )
    }
...
def generate_doctest_report(results: Dict[str, Any], 
    ...
    """
    report_lines = [
        "SolarWindPy Doctest Physics Validation Report",
        "=" * 45,
        ""
    ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Run doctests with physics validation'
    )
    parser.add_argument(
        'path', 
        help='Path to Python file or directory to test'
    )
    parser.add_argument(
        '--output-report', 
    ...
    results = run_enhanced_doctests(
        ...
        physics_validation=not args.no_physics
    )
