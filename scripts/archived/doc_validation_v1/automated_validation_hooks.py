...
class ValidationHooks:
    ...
    def __init__(self, project_root: Optional[str] = None):
        ...
        self.multiindex_validator = self.project_root / "multiindex_structure_validator.py"
        ...
        for validator_type in ['physics', 'multiindex']:
            ...
            if not result['success']:
                if result['critical']:
                    ...
                    print(f"⚠️  {validator_type.title()} validation FAILED (non-critical)")
            else:
    ...
    def github_actions_hook(self) -> int:
        ...
        if summary_file:
            with open(summary_file, 'w') as f:
                ...
                f.write(f"**Files Validated**: {results['files_validated']}\\n")
                f.write(f"**Overall Status**: {'✅ PASSED' if results['overall_success'] else '❌ FAILED'}\\n\\n")
...
def main():
    ...
    parser.add_argument(
        '--hook',
        choices=['precommit', 'github-actions'],
        help='Run specific hook type'
    )
    parser.add_argument(
        '--validate-changed',
        action='store_true',
        help='Validate only changed files'
    )
    parser.add_argument(
        '--files',
        nargs='*',
