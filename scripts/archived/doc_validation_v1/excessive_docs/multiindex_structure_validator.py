...
class MultiIndexStructureValidator:
    ...
    def __init__(self, tolerance: float = 0.05, strict_mode: bool = False):
        ...
        self.valid_components = {
            ...
            'par', 'per',     # Parallel/perpendicular
            'mag',            # Magnitude
        ...
        self.valid_species = {
            'p1', 'p2',       # Protons (primary/secondary)
            'a',              # Alpha particles  
            'he', 'he2',      # Helium
            'o', 'o6', 'o7',  # Oxygen
            'c', 'c6',        # Carbon
    ...
    def validate_structure(self, data: Dict[str, Any], 
                         file_path: str = None) -> ValidationReport:
        """
        ...
        for var_name, value in data.items():
            if isinstance(value, pd.DataFrame):
                ...
                violations.extend(self._validate_dataframe_structure(
                    value, var_name, file_path
                ))
    ...
    def validate_code_patterns(self, code: str, 
                              file_path: str = None) -> ValidationReport:
        """
    ...
    def _validate_dataframe_structure(self, df: pd.DataFrame, 
                                    var_name: str, 
                                    file_path: str = None) -> List[StructureViolation]:
        ...
        if not isinstance(df.columns, pd.MultiIndex):
            ...
            physics_cols = any(col in str(df.columns).lower() 
                             for col in ['velocity', 'density', 'temperature', 
                                       'magnetic', 'plasma', 'proton'])
    ...
    def _check_multiindex_levels(self, df: pd.DataFrame, var_name: str,
                                file_path: str = None) -> List[StructureViolation]:
    ...
    def _check_multiindex_values(self, df: pd.DataFrame, var_name: str,
                                file_path: str = None) -> List[StructureViolation]:
        ...
        violations.extend(self._check_vector_scalar_consistency(df, var_name, file_path))
        
        return violations
    
    def _check_vector_scalar_consistency(self, df: pd.DataFrame, var_name: str,
                                        file_path: str = None) -> List[StructureViolation]:
        ...
        scalar_measurements = {'n', 'T', 'P'}  # Density, temperature, pressure are scalars
    ...
    def _check_index_structure(self, df: pd.DataFrame, var_name: str,
                              file_path: str = None) -> List[StructureViolation]:
    ...
    def _check_memory_efficiency(self, df: pd.DataFrame, var_name: str,
                               file_path: str = None) -> List[StructureViolation]:
    ...
    def _check_multiindex_creation_patterns(self, code: str,
                                          file_path: str = None) -> List[StructureViolation]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if 'MultiIndex.from_tuples' in line:
                ...
                context_lines = lines[max(0, i-2):min(len(lines), i+10)]  # Look further ahead
                ...
                elif "names=['M', 'C', 'S']" not in context and 'names=["M", "C", "S"]' not in context:
                    violations.append(StructureViolation(
            ...
            if ('(' in line and ')' in line and 
                ('from_tuples' in line or 
                 (i < len(lines) - 1 and 'from_tuples' in lines[i]) or
                 (i > 0 and 'from_tuples' in lines[i-2]))):
                # Count tuple elements - should be multiples of 3
                tuple_elements = line.count("'") + line.count('"')
                if tuple_elements > 0 and tuple_elements % 6 != 0:  # Each tuple needs 6 quotes (3 elements)
                    violations.append(StructureViolation(
    ...
    def _check_data_access_patterns(self, code: str,
                                  file_path: str = None) -> List[StructureViolation]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if '.xs(' in line:
                if 'level=' in line:
                    ...
                    if not any(level in line for level in ["'M'", "'C'", "'S'", '"M"', '"C"', '"S"']):
                        violations.append(StructureViolation(
    ...
    def _check_index_naming_patterns(self, code: str,
                                   file_path: str = None) -> List[StructureViolation]:
    ...
    def generate_report(self, report: ValidationReport, verbose: bool = False) -> str:
        ...
        if report.violations_by_rule:
            ...
            sorted_rules = sorted(report.violations_by_rule.items(), 
        ...
        if verbose and report.violations:
            ...
            for i, violation in enumerate(report.violations, 1):
                icon = "🔴" if violation.severity == 'error' else "🟡" if violation.severity == 'warning' else "🔵"
        ...
        if not report.compliant:
            ...
            warning_rules = {v.rule for v in report.violations if v.severity == 'warning'}
            
            if 'missing_multiindex' in error_rules:
                lines.append("• Convert physics DataFrames to use MultiIndex structure")
                lines.append("  Example: pd.MultiIndex.from_tuples([...], names=['M', 'C', 'S'])")
            ...
            if 'xs_missing_level' in [v.rule for v in report.violations]:
                lines.append("• Use .xs() with level names for efficient MultiIndex access")
                lines.append("  Example: df.xs('v', level='M') instead of complex indexing")
...
Examples:
  ...
  %(prog)s test_file.py --verbose --output report.txt
    ...
    parser.add_argument(
        '--patterns', nargs='*',
        help='Glob patterns to match multiple files'
    )
    parser.add_argument(
    ...
    if args.output:
        ...
        try:
            summary_end = next(i for i, line in enumerate(lines) if line.strip() == "" and i > 10)
