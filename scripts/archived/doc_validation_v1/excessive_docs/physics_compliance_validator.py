...
class ThermalSpeedComplianceChecker:
    ...
    def check_code_patterns(self, code: str, file_path: str = None) -> List[ComplianceViolation]:
    ...
    def _check_thermal_speed_formulas(self, line: str, line_num: int, file_path: str) -> List[ComplianceViolation]:
    ...
    def _check_temperature_consistency(self, line: str, line_num: int, file_path: str) -> List[ComplianceViolation]:
        ...
        for var_name, value in data.items():
            if isinstance(value, pd.DataFrame) and hasattr(value, 'columns'):
                if isinstance(value.columns, pd.MultiIndex):
                    violations.extend(self._validate_multiindex_thermal_speeds(value, var_name))
            elif isinstance(value, (np.ndarray, pd.Series)) and 'thermal' in var_name.lower():
                violations.extend(self._validate_thermal_speed_array(value, var_name))
        
        return violations
    
    def _validate_multiindex_thermal_speeds(self, df: pd.DataFrame, var_name: str) -> List[ComplianceViolation]:
            ...
            for w_col in w_cols:
                ...
                if T_col is not None:
                    ...
                    if len(w_data) > 0 and len(T_data) > 0:
                        ...
                        mass = PhysicsConstants.species_masses.get(species, PhysicsConstants.m_p)
                        violations.extend(self._check_thermal_speed_temperature_relation(
    ...
    def _validate_thermal_speed_array(self, data: Union[np.ndarray, pd.Series], var_name: str) -> List[ComplianceViolation]:
    ...
    def _check_thermal_speed_temperature_relation(self, w_data: np.ndarray, T_data: np.ndarray, 
                                                 mass: float, var_name: str, species: str) -> List[ComplianceViolation]:
...
class UnitsComplianceChecker:
    ...
    def _check_unit_conversions(self, line: str, line_num: int, file_path: str) -> List[ComplianceViolation]:
    ...
    def _check_hardcoded_values(self, line: str, line_num: int, file_path: str) -> List[ComplianceViolation]:
    ...
    def _check_variable_units(self, var_name: str, data: Any) -> List[ComplianceViolation]:
        ...
        if any(b_key in var_name.lower() for b_key in ['magnetic', 'b_field', 'bfield']):
...
class MissingDataComplianceChecker:
    ...
    def _check_fill_value_usage(self, line: str, line_num: int, file_path: str) -> List[ComplianceViolation]:
        ...
        if '= 0' in line and any(phys in line.lower() for phys in ['temperature', 'density', 'pressure']):
            violations.append(ComplianceViolation(
    ...
    def _check_nan_usage(self, line: str, line_num: int, file_path: str) -> List[ComplianceViolation]:
    ...
    def _check_data_for_fill_values(self, var_name: str, data: Any) -> List[ComplianceViolation]:
...
class AlfvenSpeedComplianceChecker:
    ...
    def _check_alfven_formulas(self, line: str, line_num: int, file_path: str) -> List[ComplianceViolation]:
    ...
    def validate_data_values(self, data: Dict[str, Any]) -> List[ComplianceViolation]:
        ...
        for var_name, value in data.items():
            if any(alfven_key in var_name.lower() for alfven_key in ['alfven', 'v_a', 'ca']):
                violations.extend(self._validate_alfven_speed_values(value, var_name))
        
        return violations
    
    def _validate_alfven_speed_values(self, data: Any, var_name: str) -> List[ComplianceViolation]:
...
class MultiIndexComplianceChecker:
    ...
    def _check_multiindex_creation(self, code: str, file_path: str) -> List[ComplianceViolation]:
    ...
    def _check_multiindex_usage(self, line: str, line_num: int, file_path: str) -> List[ComplianceViolation]:
    ...
    def _validate_multiindex_structure(self, df: pd.DataFrame, var_name: str) -> List[ComplianceViolation]:
...
class PhysicsComplianceValidator:
    ...
    def validate_documentation_examples(self, doc_path: Union[str, Path]) -> ValidationResult:
        """
    ...
    def validate_multiple_files(self, file_patterns: List[str], 
                               base_path: Union[str, Path] = '.') -> ValidationResult:
        """
    ...
    def _validate_code_static(self, code: str, file_path: str) -> List[ComplianceViolation]:
        """Perform static code analysis for physics compliance."""
        violations = []
        
        # Run all static checkers
        violations.extend(self.thermal_speed_checker.check_code_patterns(code, file_path))
        violations.extend(self.units_checker.check_code_patterns(code, file_path))
        violations.extend(self.missing_data_checker.check_code_patterns(code, file_path))
        violations.extend(self.alfven_speed_checker.check_code_patterns(code, file_path))
        violations.extend(self.multiindex_checker.check_code_patterns(code, file_path))
        
        return violations
    
    def _validate_code_dynamic(self, code: str, file_path: str) -> List[ComplianceViolation]:
        ...
        try:
            ...
            for checker in [self.thermal_speed_checker, self.units_checker, 
                           self.missing_data_checker, self.alfven_speed_checker,
    ...
    def generate_report(self, result: ValidationResult) -> str:
        ...
        if result.violations:
            ...
            for i, violation in enumerate(result.violations[:20]):  # Limit output
                report.append(f"{i+1}. {violation.severity.upper()}: {violation.message}")
...
def main():
    ...
    parser.add_argument(
        'files', nargs='*', default=['docs/source/usage.rst'],
    ...
    if all_results:
        ...
        combined_result = ValidationResult(
            compliant=len([v for v in combined_violations if v.severity == 'error']) == 0,
