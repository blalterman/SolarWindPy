...
class ThermalSpeedValidator:
    ...
    def validate_thermal_speed(self, thermal_speed: Union[float, np.ndarray], 
                              temperature: Union[float, np.ndarray],
                              mass: float = PhysicsConstants.m_p) -> List[PhysicsViolation]:
        """
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", RuntimeWarning)  # Ignore div by zero warnings
        ...
        try:
            ...
            if ('w', '', 'p1') in data.columns and ('T', '', 'p1') in data.columns:
                ...
                if len(thermal_speed) > 0 and len(temperature) > 0:
                    violations.extend(self.validate_thermal_speed(
...
class UnitsValidator:
    ...
    def _check_velocity_units(self, var_name: str, data: np.ndarray) -> List[PhysicsViolation]:
    ...
    def _check_density_units(self, var_name: str, data: np.ndarray) -> List[PhysicsViolation]:
    ...
    def _check_magnetic_field_units(self, var_name: str, data: np.ndarray) -> List[PhysicsViolation]:
...
class MissingDataValidator:
    ...
    def _check_pandas_missing_data(self, var_name: str, data: Union[pd.Series, pd.DataFrame]) -> List[PhysicsViolation]:
    ...
    def _check_numpy_missing_data(self, var_name: str, data: np.ndarray) -> List[PhysicsViolation]:
...
class MultiIndexValidator:
    ...
    def validate_multiindex_structure(self, data: Dict[str, Any]) -> List[PhysicsViolation]:
    ...
    def _check_multiindex_levels(self, var_name: str, df: pd.DataFrame) -> List[PhysicsViolation]:
    ...
    def _check_multiindex_values(self, var_name: str, df: pd.DataFrame) -> List[PhysicsViolation]:
...
class PhysicsValidator:
    ...
    def validate_outputs(self, outputs: Dict[str, Any]) -> List[PhysicsViolation]:
        ...
        violations.extend(self.multiindex_validator.validate_multiindex_structure(outputs))
        
        # Look for plasma DataFrames for specialized validation
        for var_name, value in outputs.items():
            if isinstance(value, pd.DataFrame) and isinstance(value.columns, pd.MultiIndex):
                violations.extend(self.thermal_speed_validator.validate_from_plasma_data(value))
    ...
    def generate_report(self, violations: List[PhysicsViolation]) -> Dict[str, Any]:
        ...
        return {
            'physics_compliant': len([v for v in violations if v.severity == 'error']) == 0,
