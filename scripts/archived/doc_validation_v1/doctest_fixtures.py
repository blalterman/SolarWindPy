...
def create_example_plasma_data(epoch: Optional[pd.DatetimeIndex] = None, 
                             n_points: int = 10,
                             seed: int = 42,
                             include_alphas: bool = False) -> pd.DataFrame:
    ...
    with warnings.catch_warnings():
        ...
        if include_alphas:
            ...
            T_a = np.maximum(np.random.normal(4*T_p, T_p, n_points), 1e4)  # [K] - typically hotter
...
def create_example_ion_data(species: str = 'p1', 
    ...
    try:
        ...
        raise ValueError(f"Species '{species}' not found in generated data. "
                        f"Available species: {list(full_data.columns.get_level_values('S').unique())}")


def create_example_magnetic_field(epoch: Optional[pd.DatetimeIndex] = None,
                                n_points: int = 10,
                                seed: int = 42,
                                field_type: str = 'parker_spiral') -> pd.DataFrame:
...
def create_test_plasma_object(epoch: Optional[pd.DatetimeIndex] = None,
                            n_points: int = 10,
                            seed: int = 42,
...
def generate_physics_compliant_values(measurement_type: str,
                                    n_points: int = 10,
...
def validate_physics_relationships(data: pd.DataFrame,
    ...
    try:
        ...
        results['positive_quantities'] = all(positive_quantities) if positive_quantities else True
        ...
        for component in ['x', 'y', 'z']:
            if ('v', component, 'p1') in data.columns:
                ...
                if component == 'x':  # Bulk flow component
                    velocity_reasonable &= (200 <= v_comp).all() and (v_comp <= 800).all()
                else:  # Perpendicular components
                    velocity_reasonable &= (-200 <= v_comp).all() and (v_comp <= 200).all()
...
def create_doctest_summary_report() -> Dict[str, Any]:
    ...
    return {
        ...
        'validation_functions': [
            'validate_physics_relationships'
        ],
        'physics_constants': {
            'k_B': 1.380649e-23,      # Boltzmann constant [J/K]
            'm_p': 1.67262192e-27,    # Proton mass [kg]
