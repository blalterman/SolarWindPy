...
class DynamicImportTester:
    ...
    def test_individual_import(self, module_name: str) -> Dict:
        ...
        try:
            ...
            for module in new_modules:
                if (module.startswith(self.package_name) and 
    ...
    def test_import_order_variations(self, modules: List[str]) -> Dict:
        ...
        for order_name in import_orders:
            ...
            if order_name == 'alphabetical':
                ...
                base_modules = [m for m in modules if any(x in m for x in ['base', 'core', '__init__'])]
                ...
                test_modules = test_modules[:min(10, len(modules))]  # Sample for performance
        ...
        for module_name in modules:
            try:
                ...
                error_info = {
                    'module': module_name,
                    'error': str(e),
                    'is_circular': 'circular' in str(e).lower() or 'cannot import name' in str(e).lower()
                }
    ...
    def test_cross_imports(self, modules: List[str]) -> Dict:
        ...
        key_modules = [m for m in modules if any(x in m for x in [
            'core', '__init__', 'plotting', 'fitfunctions', 'base'
        ])]
    ...
    def run_comprehensive_test(self) -> Dict:
        ...
        # Generate summary
        successful_individual = sum(1 for r in results['individual_import_tests'] if r['success'])
        failed_individual = len(results['individual_import_tests']) - successful_individual
        circular_individual = sum(1 for r in results['individual_import_tests'] if r['circular_import'])
        ...
        results['summary'] = {
            ...
            'cross_import_circular_errors': len(results['cross_import_tests'].get('circular_import_errors', [])),
            'total_circular_imports_detected': len(results['circular_imports_found'])
        }
        
        return results
    
    def save_report(self, results: Dict, output_file: str = "dynamic_import_test_report.txt"):
        ...
        with open(output_file, 'w') as f:
            ...
            f.write(f"Individual imports successful: {summary['individual_imports_successful']}\n")
            f.write(f"Individual imports failed: {summary['individual_imports_failed']}\n")
            f.write(f"Circular imports detected: {summary['total_circular_imports_detected']}\n\n")
            ...
            f.write(f"Total cross-import tests: {cross_results.get('total_tests', 0)}\n")
            ...
            f.write(f"Circular import errors: {len(cross_results.get('circular_import_errors', []))}\n")
            
            if cross_results.get('circular_import_errors'):
                f.write("\nCross-import circular errors:\n")
                for error in cross_results['circular_import_errors']:
                    f.write(f"  {error['first_module']} + {error['second_module']}: {error['error']}\n")
    ...
    def print_summary(self, results: Dict):
        ...
        print(f"Circular imports detected: {summary['total_circular_imports_detected']}")
