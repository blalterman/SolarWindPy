...
class CIIntegration:
    ...
    def get_validation_files(self) -> List[str]:
        ...
        key_files = ['docs/source/usage.rst', 'docs/source/tutorial/quickstart.rst', 'docs/source/installation.rst',
                    'solarwindpy/core/plasma.py', 'solarwindpy/core/ions.py', 'solarwindpy/tools/__init__.py']
        
        return [str(self.project_root / f) for f in key_files if (self.project_root / f).exists()]
    ...
    def run_validation(self, files: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        results = {'files_processed': 0, 'total_tests': 0, 'failed_tests': 0, 'examples_run': 0, 'files': [], 'overall_success': True}
    ...
    def validate_rst_examples(self, rst_file: Path) -> Dict[str, Any]:
        ...
        result = {'file': str(rst_file), 'tests_attempted': 0, 'tests_failed': 0, 'examples_run': 0, 'success': True, 'errors': []}
    ...
    def create_github_outputs(self, results: Dict[str, Any]) -> None:
        ...
        if output_file:
            with open(output_file, 'a') as f:
                f.write(f"validation-success={str(results['overall_success']).lower()}\n")
        ...
        if summary_file:
            ...
            with open(summary_file, 'w') as f:
                ...
                f.write(f"**Files**: {results['files_processed']}, **Tests**: {results['total_tests']}, **Status**: {status}\n")
    ...
    def save_artifacts(self, results: Dict[str, Any]) -> None:
        ...
        status_msg = "All validations passed" if results['overall_success'] else "Validation failures detected"
        with open(self.project_root / "doctest_validation_summary.txt", 'w') as f:
            f.write(f"SolarWindPy Doctest Validation Summary\n")
            f.write(f"Files: {results['files_processed']}, Tests: {results['total_tests']}, Failed: {results['failed_tests']}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='CI integration for doctest validation')
    parser.add_argument('--github-actions', action='store_true', 
                       help='Run GitHub Actions workflow')
