...
class SimpleDocTestRunner:
    ...
    def __init__(self, verbose: bool = False):
        ...
        self.results = {'files_processed': 0, 'total_tests': 0, 'failed_tests': 0, 'examples_run': 0, 'files': []}
    
    def run_file(self, file_path: Path) -> Dict[str, Any]:
        """Run doctests on a single Python file"""
        result = {'file': str(file_path), 'tests_attempted': 0, 'tests_failed': 0, 'examples_run': 0, 'success': True, 'errors': []}
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        success_rate = ((self.results['total_tests'] - self.results['failed_tests']) / max(self.results['total_tests'], 1)) * 100
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Simple doctest runner for SolarWindPy')
    ...
    if path.is_dir():
        ...
        runner.results = {'files_processed': 1, 'total_tests': result['tests_attempted'], 
                         'failed_tests': result['tests_failed'], 'examples_run': result['examples_run'], 'files': [result]}
...
"""
    ...
    for file_info in summary['files']:
        ...
        text_report += f"{status} {file_info['file']} ({file_info['tests_attempted']} tests)\n"
