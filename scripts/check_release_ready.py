...
def check_branch() -> Tuple[bool, str]:
    ...
    if branch in ['master', 'main']:
        ...
        return False, f"On feature branch '{branch}'. Switch to master/main for release."
    else:
...
def check_setuptools_scm() -> Tuple[bool, str]:
    ...
    try:
        ...
        version = get_version()
        if 'dev' in version:
            return False, f"Development version detected: {version}. Create a tag for release."
...
def check_changelog() -> Tuple[bool, str]:
    ...
    if '## [Unreleased]' not in content:
        return False, "CHANGELOG.md missing [Unreleased] section. Update to Keep a Changelog format."
    ...
    if unreleased_match and unreleased_match.group(1).strip():
        return False, "CHANGELOG.md has unreleased changes. Move to versioned section before release."
...
def check_linting() -> Tuple[bool, str]:
    ...
    if code != 0:
        return False, f"Linting errors found. Fix with 'black .' and address flake8 issues:\n{stderr}"
...
def check_pypi_tokens() -> Tuple[bool, str]:
    ...
    return True, "PyPI tokens will be validated during deployment (10-day delay expected)"
...
def print_summary(results: Dict[str, Tuple[bool, str]]):
    ...
    if failed == 0:
        ...
        print(f"{Colors.GREEN}All {total} checks passed. You can proceed with release creation.{Colors.END}")
        print(f"\n{Colors.BLUE}Next steps:{Colors.END}")
        print(f"  1. Create release tag: python scripts/bump_version.py [major|minor|patch|rc]")
