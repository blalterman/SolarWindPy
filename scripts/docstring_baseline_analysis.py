...
class DocstringFormatAnalyzer(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        ...
        if docstring:
            ...
            format_type = self.analyze_docstring_format(docstring, 'function', node.name)
        else:
            self.issues.append({
        ...
        for item in node.body:
            if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                if method_docstring:
                    ...
                    self.analyze_docstring_format(method_docstring, 'method', 
...
def main():
    ...
    print(f"  Total Code Objects: {total_stats['total_functions'] + total_stats['total_classes'] + total_stats['total_methods']}")
    print()
    
    print("Documentation Coverage:")
    total_objects = total_stats['total_functions'] + total_stats['total_classes'] + total_stats['total_methods']
    documented_objects = total_stats['functions_with_docstrings'] + total_stats['classes_with_docstrings'] + total_stats['methods_with_docstrings']
    coverage_pct = (documented_objects / total_objects * 100) if total_objects > 0 else 0
    print(f"  Functions with docstrings: {total_stats['functions_with_docstrings']}/{total_stats['total_functions']} ({total_stats['functions_with_docstrings']/total_stats['total_functions']*100:.1f}%)")
    print(f"  Classes with docstrings: {total_stats['classes_with_docstrings']}/{total_stats['total_classes']} ({total_stats['classes_with_docstrings']/total_stats['total_classes']*100:.1f}%)")
    print(f"  Methods with docstrings: {total_stats['methods_with_docstrings']}/{total_stats['total_methods']} ({total_stats['methods_with_docstrings']/total_stats['total_methods']*100:.1f}%)")
    print(f"  Overall Coverage: {documented_objects}/{total_objects} ({coverage_pct:.1f}%)")
    print()
    
    print("Format Distribution:")
    total_documented = total_stats['numpy_format'] + total_stats['google_format'] + total_stats['informal_format']
    if total_documented > 0:
        print(f"  NumPy format: {total_stats['numpy_format']} ({total_stats['numpy_format']/total_documented*100:.1f}%)")
        print(f"  Google format: {total_stats['google_format']} ({total_stats['google_format']/total_documented*100:.1f}%)")
        print(f"  Informal format: {total_stats['informal_format']} ({total_stats['informal_format']/total_documented*100:.1f}%)")
    ...
    print(f"- {total_stats['google_format'] + total_stats['informal_format']} items need format conversion")
