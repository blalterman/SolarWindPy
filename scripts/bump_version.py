...
def get_latest_version() -> Optional[str]:
    ...
    code, stdout, stderr = run_command(['git', 'tag', '-l', 'v*', '--sort=-version:refname'])
    if code != 0:
    ...
    release_tags = [tag for tag in tags if tag.startswith('v') and not 'compaction' in tag]
...
def parse_version(version_string: str) -> Tuple[int, int, int, Optional[str], Optional[int]]:
...
def bump_version(current_version: str, bump_type: str) -> str:
    ...
    major, minor, patch, prerelease_type, prerelease_num = parse_version(current_version)
...
def check_working_directory() -> bool:
    ...
    if stdout:
        ...
        print(stdout)
        print(f"{Colors.YELLOW}Commit or stash changes before creating a release tag.{Colors.END}")
        return False
...
def create_tag(tag_name: str, dry_run: bool = False) -> bool:
    ...
    print(f"  2. Monitor GitHub Actions: https://github.com/blalterman/SolarWindPy/actions")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Bump SolarWindPy version following semantic versioning')
    parser.add_argument('bump_type', choices=['major', 'minor', 'patch', 'rc', 'beta', 'alpha'],
    ...
    if args.from_version:
        ...
        if not current_version:
            print(f"{Colors.YELLOW}No existing version tags found. Starting from v0.0.0{Colors.END}")
    ...
    if not validate_version_progression(current_version, new_version):
        print(f"{Colors.RED}Error: Invalid version progression from {current_version} to {new_version}{Colors.END}")
        sys.exit(1)
    
    # Show version details
    try:
        major, minor, patch, prerelease_type, prerelease_num = parse_version(new_version)
