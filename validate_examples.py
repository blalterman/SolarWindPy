...
class ValidationReporter:
    ...
    def add_result(self, example: CodeExample, execution_result: ExecutionResult, 
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'common_error_types': dict(sorted(error_types.items(), key=lambda x: x[1], reverse=True)),
            'average_execution_time': avg_execution_time,
            'examples_with_output': sum(1 for r in self.results if r['outputs_count'] > 0)
        }
...
class ExampleValidator:
    ...
    def validate_example(self, example: CodeExample) -> Dict[str, Any]:
        ...
        if example.example_type == 'bash':
            return {
                'skipped': True, 
                'reason': 'Bash examples not currently supported'
            }
    ...
    def validate_all_from_inventory(self, inventory_path: Path) -> None:
        ...
        print(f"Successful: {summary['successful_executions']} ({summary.get('success_rate', 0):.1f}%)")
