...
class SolarWindPyPlanValueValidator:
    ...
    def __init__(self):
        self.required_sections = {
            ...
            'security_proposition': {
                ...
                'forbidden_content': [
                    'fair', 'metadata security', 'persistent identifier',
    ...
    def _validate_section(self, content: str, section_key: str, section_config: Dict, strict: bool) -> Dict:
        ...
        section_content = self._extract_section_content(content, section_config['pattern'])
        ...
        if 'required_subsections' in section_config:
            for subsection in section_config['required_subsections']:
                if subsection.lower() not in section_content.lower():
                    if strict:
                        result['passed'] = False
                        result['issues'].append(f"Missing required subsection: {subsection}")
                    else:
                        result['warnings'].append(f"Recommended subsection missing: {subsection}")
    ...
    def _validate_security_section(self, section_content: str, strict: bool) -> Dict:
        ...
        required_components = {
            ...
            'attack_surface': ['exposure', 'interface', 'attack', 'surface', 'validation'],
            'development_security': ['workflow', 'ci/cd', 'development', 'git', 'review']
        }
        ...
        scope_indicators = [
            'code-level', 'explicitly excluded', 'not implemented', 'outside scope'
        ]

        if not any(indicator in section_content.lower() for indicator in scope_indicators):
        ...
        if 'fair' in section_content.lower():
            fair_exclusion_phrases = [
                'excluded', 'not implemented', 'outside scope', 'not applicable'
            ]
            if not any(phrase in section_content.lower() for phrase in fair_exclusion_phrases):
                result['issues'].append(
    ...
    def _validate_token_section(self, section_content: str, strict: bool) -> Dict:
        ...
        token_numbers = re.findall(self.token_patterns['token_numbers'], section_content)

        if not token_numbers:
            result['issues'].append("No token usage numbers found in token optimization section")
            return result
        ...
        for token_str in token_numbers:
            ...
            if tokens < 50:
                result['warnings'].append(f"Token estimate {tokens} seems unusually low")
            elif tokens > 15000:
                result['issues'].append(f"Token estimate {tokens} seems unrealistically high")

        # Check for savings percentage
        savings_matches = re.findall(self.token_patterns['token_savings'], section_content)
        ...
        comparison_indicators = ['current', 'optimized', 'manual', 'automated', 'vs', 'versus']
        if not any(indicator in section_content.lower() for indicator in comparison_indicators):
            result['warnings'].append(
    ...
    def _validate_time_section(self, section_content: str, strict: bool) -> Dict:
        ...
        time_estimates = re.findall(self.token_patterns['time_estimates'], section_content)

        if not time_estimates:
            result['warnings'].append("No specific time estimates found in time analysis")

        # Check for required components
        required_time_components = [
            'implementation', 'breakdown', 'savings', 'break-even'
        ]
    ...
    def _generate_summary(self, results: Dict) -> Dict:
        ...
        passed_sections = sum(1 for section in results['sections'].values() if section['passed'])

        total_issues = sum(len(section['issues']) for section in results['sections'].values())
        total_warnings = sum(len(section['warnings']) for section in results['sections'].values())
        ...
        return {
            ...
            'recommendation': self._get_recommendation(results['valid'], total_issues, total_warnings)
        }

    def _get_recommendation(self, valid: bool, total_issues: int, total_warnings: int) -> str:
    ...
    def generate_report(self, validation_results: Dict, format: str = 'text') -> str:
        ...
        report_lines.append(f"**Strict Mode**: {'Yes' if validation_results['strict_mode'] else 'No'}")
        report_lines.append("")

        # Overall status
        status_emoji = "✅" if validation_results['valid'] else "❌"
        report_lines.append(f"## {status_emoji} Overall Status: {validation_results['summary']['overall_status']}")
        report_lines.append(f"- **Sections Passed**: {validation_results['summary']['sections_passed']}")
        report_lines.append(f"- **Total Issues**: {validation_results['summary']['total_issues']}")
        report_lines.append(f"- **Total Warnings**: {validation_results['summary']['total_warnings']}")
        report_lines.append(f"- **Completion**: {validation_results['summary']['completion_percentage']:.0f}%")
        report_lines.append(f"- **Recommendation**: {validation_results['summary']['recommendation']}")
        report_lines.append("")
        ...
        for section_key, section_result in validation_results['sections'].items():
            ...
            quality_indicator = {"good": "👍", "minimal": "⚠️", "poor": "👎", "verbose": "📝"}
            quality_emoji = quality_indicator.get(section_result['content_quality'], "❓")

            report_lines.append(f"### {section_emoji} {section_result['section_name']} {quality_emoji}")
            ...
            if not section_result['passed']:
                ...
                for issue in section_result['issues']:
                    if 'not found' in issue:
                        suggestions.append(f"- Add the missing '{section_name}' section to your plan")
                    elif 'is empty' in issue:
                        suggestions.append(f"- Add content to the '{section_name}' section")
                    elif 'FAIR compliance' in issue:
                        suggestions.append("- Remove FAIR data references from security section")
                    elif 'Missing' in issue and 'subsection' in issue:
                        missing_subsection = issue.split(': ')[1] if ': ' in issue else 'subsection'
                        suggestions.append(f"- Add '{missing_subsection}' subsection to '{section_name}'")
                    elif 'token' in issue.lower():
                        suggestions.append("- Add specific token usage numbers and savings percentages")
                    elif 'time' in issue.lower():
                        suggestions.append("- Include time estimates and implementation breakdown")

        # Overall suggestions
        for issue in validation_results['overall_issues']:
            if 'placeholder' in issue.lower():
                suggestions.append("- Replace all placeholder content with actual generated content")
            elif 'fair' in issue.lower():
                suggestions.append("- Limit FAIR data mentions to security section exclusion only")
    ...
    def can_complete_plan(self, plan_file: Path) -> Tuple[bool, str]:
        """Check if plan can be marked as completed (integration with plan-completion-manager)."""
        ...
        if validation_results['valid']:
            ...
            return False, f"Value proposition validation failed: {'; '.join(issues_summary[:3])}"
