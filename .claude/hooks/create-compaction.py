...
def get_git_info():
    ...
    try:
        ...
        commits = subprocess.check_output(
            ['git', 'log', '--oneline', '-5'], text=True).strip()

        # Get current status
        status = subprocess.check_output(
            ['git', 'status', '--short'], text=True).strip()
...
def get_enhanced_git_info():
    ...
    try:
        ...
        commits = subprocess.check_output(['git', 'log', '--oneline', '--decorate', '-5'], text=True).strip()
        ...
        diff_stats = ""
        try:
            diff_output = subprocess.check_output(['git', 'diff', '--stat'], text=True).strip()
            if diff_output:
        ...
        last_commit_info = ""
        try:
            last_commit_info = subprocess.check_output(['git', 'log', '-1', '--pretty=format:%h - %s (%an, %ar)'], text=True).strip()
        except subprocess.CalledProcessError:
            pass
            ...
            for i, line in enumerate(reversed(lines[:50])):  # Check last 50 lines
                if 'Implementation Notes' in line or 'Progress' in line:
                    ...
                    critical_content['recent_decisions'] = [
                        line.strip() for line in recent_notes
    ...
    for phase_file in phase_files[:2]:  # Check 2 most recent
        try:
            ...
            if 'blocker' in content.lower() or 'issue' in content.lower():
                lines = content.splitlines()
                for line in lines:
                    if any(word in line.lower() for word in ['blocker', 'issue', 'problem']):
            ...
            if 'next' in content.lower() or 'todo' in content.lower():
                lines = content.splitlines()
                for line in lines:
                    if any(word in line.lower() for word in ['next', 'todo', 'remaining']):
...
def create_compaction():
    ...
    branch, commits, status = git_info['branch'], git_info['commits'], git_info['status']
    ...
    if compression_strategy['compress_git']:
        commits, status = compress_git_history(commits, status, compression_strategy['level'])
...
"""
    ...
    if critical_context['active_tasks']:
        priority_actions.extend([f"Continue: {task}" for task in critical_context['active_tasks'][:2]])

    if critical_context['blockers']:
        priority_actions.extend([f"Resolve: {blocker}" for blocker in critical_context['blockers'][:2]])

    # Add git status command if there are uncommitted changes
    if status:
        quick_commands.append("git status")
        priority_actions.append("Review uncommitted changes and decide on commit strategy")
