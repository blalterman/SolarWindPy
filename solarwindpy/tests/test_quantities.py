...
class QuantityTestBase(ABC):
    ...
    def test_eq(self):
        ...
        if print_inline_debug:
            print("<Test>",
                  "<object_testing>",
                  type(object_testing), object_testing, object_testing.data,
                  "<new_object>",
                  type(new_object),
                  new_object,
                  new_object.data,
    ...
    def test_empty_data_catch(self):
        ...
        with self.assertRaisesRegex(ValueError,
                                    "You can't set an object with empty data."):
...
class VectorTestBase(QuantityTestBase):
    ...
    def test_mag(self):
        ...
        pdt.assert_series_equal(self.object_testing.mag,
                                self.object_testing.magnitude)
    def test_rho(self):
        # print("test_rho")
        x = self.data.x
        y = self.data.y
    ...
    def test_longitude(self):
        ...
        pdt.assert_series_equal(self.object_testing.lon,
                                self.object_testing.longitude)
    def test_r(self):
        # print("test_r")
        x = self.data.x
        y = self.data.y
        ...
        pdt.assert_series_equal(r, self.object_testing.mag,
                                check_names=False)
        pdt.assert_series_equal(self.object_testing.r, self.object_testing.mag,
                                check_names=False)
    ...
    def test_unit_vector(self):
        ...
        uv.name = "uv"
        uv = vector.Vector(uv)
        pdt.assert_frame_equal(uv.data,
                               self.object_testing.unit_vector.data)
        pdt.assert_frame_equal(uv.data,
                               self.object_testing.uv.data)
        pdt.assert_frame_equal(self.object_testing.uv.data,
                               self.object_testing.unit_vector.data)
        self.assertEqual(uv, self.object_testing.unit_vector)
        self.assertEqual(uv, self.object_testing.uv)
        self.assertEqual(self.object_testing.unit_vector,
                         self.object_testing.uv)
    def test_project(self):
        b = base.TestData()
        b.setUpClass()
        b = b.data.b.loc[:, ["x", "y", "z"]].xs("",
                            axis=1, level="S").xs("", axis=1, level="N")
        ...
        per = v.subtract(buv.multiply(par, axis=0),
                         axis=1).pow(2).sum(axis=1).pipe(np.sqrt)
        projected = pd.concat([par, per], axis=1,
        ...
        b = vector.Vector(b)
        pdt.assert_frame_equal(projected,
                               self.object_testing.project(b))

        # Projecting a thing onto itself should return 1 for parallel
        # and 0 for perp.
        per = pd.Series(0.0, index=per.index)
        projected = pd.concat([vmag, per], axis=1,
                              keys=["par", "per"]).sort_index(axis=1)
        pdt.assert_frame_equal(projected,
                       self.object_testing.project(self.object_testing))
    ...
    def test_cos_theta(self):
        ...
        b = vector.BField(b)
        pdt.assert_series_equal(cos_theta,
                               self.object_testing.cos_theta(b))
        ...
        pdt.assert_series_equal(par,
                                self.object_testing.cos_theta(v))
        pdt.assert_series_equal(par,
                                self.object_testing.cos_theta(vuv))
...
class TestBField(VectorTestBase, base.SWEData):
    ...
    def test_pressure(self):
        ...
        if print_inline_debug:
            ...
            print("<Module>",
                  "<object testing>", type(self.object_testing), self.object_testing,
                  "<dir(ot)>", *dir(self.object_testing),
                  sep="\n",
                  end="\n\n")

        pdt.assert_series_equal(pb, self.object_testing.pressure)
        pdt.assert_series_equal(pb, self.object_testing.pb)
        pdt.assert_series_equal(self.object_testing.pressure,
                                self.object_testing.pb)
...
class ThermalSpeedTestBase(TensorTestBase):
    ...
    def set_object_testing(cls):
        ...
        scalar = data.pow(2).multiply(coeff,
                                  axis=1, level="C").sum(axis=1).pipe(np.sqrt)
        scalar.name = "scalar"
...
class TestQuantitySubclassEquality(base.SWEData):
    ...
    def setUpClass(cls):
        ...
        cols = pd.MultiIndex.from_tuples(scalar.columns.to_series().apply(fcn),
