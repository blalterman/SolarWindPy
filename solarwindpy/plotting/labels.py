...
_all_species_re = sorted(_trans_species.keys())[::-1] # Order so we check for p1 and p2 before p.
...
class TeXlabel(object):
    ...
    def _build_one_label(self, m, c, s):
        ...
        self.logger.debug(r"""Built TeX label
TeX        : %s
with units : %s
save path  : %s
         M : %s -> %s
         ...
         S : %s -> %s""",
                        tex, with_units, path,
                        m, m1,
                        c if c else None, c1 if c1 else None,
                        s if s else None, s1 if s1 else None)
    ...
    def build_label(self, mcs0, mcs1=None):
        ...
        if mcs1 is not None:
            ...
            m0, m1 = mcs0[0], mcs1[0]
            u0, u1 = _trans_units[m0.replace("_err", "")], _trans_units[m1.replace("_err", "")]
            if u0 == u1:
                units = r"\#"
            else:
            ...
            self.logger.debug(r"""Joined ratio label
TeX       : %s
w/ units  : %s
save path :
       T0 : %s
