...
class TestAggPlotProperties:
    ...
    def test_agg_axes_property(self):
        ...
        data = pd.DataFrame({
            'x': [1, 2, 3],
            'z': [7, 8, 9]  # Only one non-groupby column
        })
...
class TestAggPlotClipData:
    ...
    def test_clip_data_series_lower(self):
        ...
        with pytest.raises(AttributeError, match="'Series' object has no attribute 'clip_lower'"):
    ...
    def test_clip_data_series_upper(self):
        ...
        with pytest.raises(AttributeError, match="'Series' object has no attribute 'clip_upper'"):
    ...
    def test_clip_data_series_both(self):
        ...
        result = AggPlot.clip_data(data, 'both')  # Any string not starting with 'l' or 'u'
    ...
    def test_clip_data_dataframe_lower(self):
        ...
        # The implementation uses deprecated clip_lower method
        with pytest.raises(AttributeError, match="'DataFrame' object has no attribute 'clip_lower'"):
            AggPlot.clip_data(data, 'l')
    ...
    def test_clip_data_dataframe_upper(self):
        ...
        # The implementation uses deprecated clip_upper method
        with pytest.raises(AttributeError, match="'DataFrame' object has no attribute 'clip_upper'"):
            AggPlot.clip_data(data, 'u')
    ...
    def test_clip_data_unsupported_type_raises_error(self):
        ...
        with pytest.raises(AttributeError, match="'list' object has no attribute 'quantile'"):
...
class TestAggPlotAggregation:
    ...
    def test_agg_with_mean_function(self):
        ...
        if len(result_clean) > 0:
            assert result_clean.min() >= data_min - 1e-10  # Allow for floating point error
