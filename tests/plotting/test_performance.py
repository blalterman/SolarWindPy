...
class TestBasicPerformance:
    ...
    def test_line_plot_performance(self):
        ...
        for size in data_sizes:
            ...
            if size <= 10000:
                assert elapsed < 1.0, f"Plot with {size} points took {elapsed:.3f}s (expected < 1.0s)"
            elif size <= 100000:
                assert elapsed < 5.0, f"Plot with {size} points took {elapsed:.3f}s (expected < 5.0s)"
            else:
                assert elapsed < 15.0, f"Plot with {size} points took {elapsed:.3f}s (expected < 15.0s)"
    ...
    def test_scatter_plot_performance(self):
        ...
        for size in data_sizes:
            ...
            if size <= 5000:
                assert elapsed < 2.0, f"Scatter with {size} points took {elapsed:.3f}s (expected < 2.0s)"
            elif size <= 25000:
                assert elapsed < 10.0, f"Scatter with {size} points took {elapsed:.3f}s (expected < 10.0s)"
            else:
                assert elapsed < 30.0, f"Scatter with {size} points took {elapsed:.3f}s (expected < 30.0s)"
    ...
    def test_histogram_performance(self):
        ...
        for size in data_sizes:
            for bins in bin_counts:
                ...
                assert elapsed < 5.0, f"Histogram with {size} points, {bins} bins took {elapsed:.3f}s"
    ...
    def test_memory_usage_scalability(self):
        ...
        for size in data_sizes:
            ...
            assert memory_increase < 100, f"Memory usage increased by {memory_increase:.1f}MB for {size} points"
...
class TestAdvancedPerformance:
    ...
    def test_contour_plot_performance(self):
        ...
        for grid_size in grid_sizes:
            for levels in contour_levels:
                ...
                expected_time = (grid_size / 50) * (levels / 10) * 2.0  # Base expectation
                assert elapsed < expected_time, f"Contour plot took {elapsed:.3f}s (expected < {expected_time:.3f}s)"
    ...
    def test_subplot_performance(self):
        ...
        for n_subplots in subplot_counts:
            ...
            assert elapsed < expected_time + 2.0, f"{n_subplots} subplots took {elapsed:.3f}s"
    ...
    def test_repeated_plot_performance(self):
        ...
        assert degradation_ratio < 2.0, f"Performance degraded by {degradation_ratio:.2f}x over {n_iterations} iterations"
        
        # Average time per iteration should be reasonable
        avg_time = np.mean(times)
        assert avg_time < 1.0, f"Average time per plot: {avg_time:.3f}s (expected < 1.0s)"
...
class TestDataTypePerformance:
    ...
    def test_pandas_integration_performance(self):
        ...
        for size in sizes:
            ...
            assert elapsed < expected_time, f"Pandas plot with {size} points took {elapsed:.3f}s"
...
class TestMemoryEfficiency:
    ...
    def test_memory_cleanup(self):
        ...
        assert memory_increase < 50, f"Memory increased by {memory_increase:.1f}MB after 20 plots"
    ...
    def test_large_figure_memory(self):
        ...
        for width, height in sizes:
            ...
            assert memory_per_area < 2.0, f"Memory per area: {memory_per_area:.3f} MB per unit²"
...
class TestLargeDatasetPerformance:
    ...
    def test_space_physics_timeseries_performance(self):
        ...
        np.random.seed(42)
        density = 10 * (1 + 0.5 * np.sin(np.arange(n_points) / (24*60)) +  # Daily variation
                       0.2 * np.sin(np.arange(n_points) / (27*24*60)) +    # Solar rotation
                       0.1 * np.random.randn(n_points))                    # Random noise
        
        velocity = 400 * (1 + 0.3 * np.sin(np.arange(n_points) / (27*24*60)) +
                         0.1 * np.random.randn(n_points))
        
        temperature = 1e5 * (1 + 0.4 * np.sin(np.arange(n_points) / (24*60)) +
        ...
        assert elapsed < 30.0, f"Large dataset plot took {elapsed:.3f}s (expected < 30s)"
    ...
    def test_high_resolution_contour_performance(self):
        ...
        assert elapsed < 60.0, f"High-res contour plot took {elapsed:.3f}s (expected < 60s)"
...
def test_memory_leak_detection():
    ...
    assert memory_increase < 20, f"Potential memory leak: {memory_increase:.1f}MB increase"
