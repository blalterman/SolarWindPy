...
def synthetic_time_series():
    ...
    data = {
        ...
        'temperature': 1e5 + 2e4 * np.sin(time_index / 80) + np.random.normal(0, 5e3, n),
...
def synthetic_plasma_data():
    ...
    velocity = 400 + 100 * (1 / density) + np.random.normal(0, 20, n_points)  # Anti-corr with density
    temperature = 1e5 * (velocity / 400)**1.5 + np.random.normal(0, 2e4, n_points)  # Corr with velocity
    b_field = 5 * np.sqrt(density) + np.random.normal(0, 1, n_points)  # Corr with density
...
def mock_axes():
    ...
    mock_ax.hist.return_value = (np.array([1, 2, 3]), np.array([0, 1, 2, 3]), [MagicMock()])
    return mock_ax
...
class PlotTestHelper:
    ...
    @staticmethod
    def assert_plot_elements(ax, expected_lines=None, expected_collections=None,
                           expected_patches=None, has_legend=None, has_grid=None):
        """Assert expected plot elements are present."""
        if expected_lines is not None:
            assert len(ax.lines) == expected_lines, f"Expected {expected_lines} lines, got {len(ax.lines)}"
        
        if expected_collections is not None:
            assert len(ax.collections) == expected_collections, f"Expected {expected_collections} collections, got {len(ax.collections)}"
        
        if expected_patches is not None:
            assert len(ax.patches) == expected_patches, f"Expected {expected_patches} patches, got {len(ax.patches)}"
        
        if has_legend is not None:
            legend_exists = ax.legend_ is not None
            assert legend_exists == has_legend, f"Legend existence mismatch: expected {has_legend}, got {legend_exists}"
        
        if has_grid is not None:
            # Check if grid is visible
            grid_on = ax.xaxis.get_gridlines()[0].get_visible() if ax.xaxis.get_gridlines() else False
            assert grid_on == has_grid, f"Grid visibility mismatch: expected {has_grid}, got {grid_on}"
    
    @staticmethod
    def assert_axes_labels(ax, xlabel=None, ylabel=None, title=None):
        """Assert axes labels are set correctly."""
        if xlabel is not None:
            assert ax.get_xlabel() == xlabel, f"X-label mismatch: expected '{xlabel}', got '{ax.get_xlabel()}'"
        
        if ylabel is not None:
            assert ax.get_ylabel() == ylabel, f"Y-label mismatch: expected '{ylabel}', got '{ax.get_ylabel()}'"
        
        if title is not None:
            assert ax.get_title() == title, f"Title mismatch: expected '{title}', got '{ax.get_title()}'"
    ...
    def assert_data_ranges(ax, xlim=None, ylim=None, tolerance=1e-10):
        ...
        if xlim is not None:
            ...
            assert abs(actual_xlim[0] - xlim[0]) < tolerance and abs(actual_xlim[1] - xlim[1]) < tolerance, \
                   f"X-limits mismatch: expected {xlim}, got {actual_xlim}"
        
        if ylim is not None:
            actual_ylim = ax.get_ylim()
            assert abs(actual_ylim[0] - ylim[0]) < tolerance and abs(actual_ylim[1] - ylim[1]) < tolerance, \
...
class PerformanceTracker:
    ...
    def assert_performance(self, operation_name, max_time):
        ...
        if avg_time is not None:
            assert avg_time < max_time, f"{operation_name} took {avg_time:.3f}s (expected < {max_time}s)"
...
class TestFixturesAndUtilities:
    ...
    def test_synthetic_plasma_data_fixture(self, synthetic_plasma_data):
        ...
        density_velocity_corr = synthetic_plasma_data['density'].corr(synthetic_plasma_data['velocity'])
        assert density_velocity_corr < 0, "Expected anti-correlation between density and velocity"
    ...
    def test_plot_helper_functionality(self, plot_helper, single_axis):
        ...
        # Test assertions
        plot_helper.assert_plot_elements(ax, expected_lines=1, has_legend=True, has_grid=True)
...
class TestFixtureIntegration:
    ...
    def test_plot_with_synthetic_data(self, single_axis, synthetic_time_series, plot_helper):
        """Test plotting with synthetic data using fixtures."""
        fig, ax = single_axis
        
        # Plot first parameter
        param = 'density'
        ax.plot(synthetic_time_series.index, synthetic_time_series[param], 
                label=param, linewidth=1.5)
        ax.set_xlabel('Time')
        ...
        plot_helper.assert_plot_elements(ax, expected_lines=1, has_legend=True, has_grid=True)
        plot_helper.assert_axes_labels(ax, xlabel='Time', ylabel='Density (cm⁻³)', 
                                     title='Synthetic Time Series Test')
    
    def test_save_plot_with_temp_files(self, single_axis, small_dataset, 
                                     temp_dir, plot_helper):
        ...
        saved_files = plot_helper.save_and_verify_plot(fig, plot_file, 
                                                     formats=['png', 'pdf'])
