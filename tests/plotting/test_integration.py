...
class TestBasicIntegrationWorkflows:
    ...
    def setup_method(self):
        ...
        # Plasma parameters
        np.random.seed(42)
        self.density = 10 + 2 * np.sin(np.arange(self.n_points) / 10) + np.random.normal(0, 0.5, self.n_points)
        self.velocity = 400 + 50 * np.sin(np.arange(self.n_points) / 15) + np.random.normal(0, 10, self.n_points)
        self.temperature = 1e5 + 2e4 * np.sin(np.arange(self.n_points) / 8) + np.random.normal(0, 5e3, self.n_points)
        self.magnetic_field = 5 + np.sin(np.arange(self.n_points) / 12) + np.random.normal(0, 0.2, self.n_points)
    ...
    def test_time_series_integration_workflow(self):
        ...
        labels_text = ['Density (cm⁻³)', 'Velocity (km/s)', 'Temperature (K)', 'B-field (nT)']
        ...
        for i, (param, label, color) in enumerate(zip(parameters, labels_text, colors)):
            ...
            axes[i].axhline(mean_val, color=color, linestyle='--', alpha=0.7, 
                          label=f'Mean: {mean_val:.2f}')
            axes[i].fill_between(self.data.index, mean_val - std_val, mean_val + std_val,
                               alpha=0.2, color=color, label=f'±1σ: {std_val:.2f}')
    ...
    def test_distribution_analysis_workflow(self):
        ...
        for i, param in enumerate(parameters):
            ...
            n, bins, patches = ax.hist(data, bins=20, alpha=0.7, density=True, 
        ...
        # Combined box plot
        ax_box = axes[1, 0]
        data_normalized = [self.data[param] / self.data[param].std() for param in parameters]
        box_plot = ax_box.boxplot(data_normalized, labels=[p.title() for p in parameters])
        ...
        for i, param in enumerate(parameters[:2]):  # Just first two to avoid clutter
            ax_stats.plot(rolling_means.index, rolling_means[param], 
                         label=f'{param.title()} (rolling mean)', linewidth=2)
        ...
        ax_scatter = axes[1, 2]
        ax_scatter.scatter(self.data['velocity'], self.data['temperature'], 
                          alpha=0.6, c='purple')
        ...
        ax_summary.text(0.1, 0.9, '\n'.join(summary_text), fontfamily='monospace',
                       fontsize=10, verticalalignment='top', transform=ax_summary.transAxes)
...
class TestLabelIntegrationWorkflow:
    ...
    def test_chemistry_labels_integration(self):
        ...
        try:
            from solarwindpy.plotting.labels.chemistry import mass_per_charge, fip, charge, mass
            ...
            axes[1, 0].bar(range(1, len(charge_counts)+1), charge_counts, 
...
class TestErrorHandlingIntegration:
    ...
    def test_missing_data_integration(self):
        ...
        with warnings.catch_warnings():
            ...
            axes[1].plot(data_interp.index, data_interp['param1'], 'b-', label='param1 (interp)')
            axes[1].plot(data_interp.index, data_interp['param2'], 'r-', label='param2 (interp)')
            ...
            axes[2].plot(data_ffill.index, data_ffill['param1'], 'b-', label='param1 (ffill)')
            axes[2].plot(data_ffill.index, data_ffill['param2'], 'r-', label='param2 (ffill)')
