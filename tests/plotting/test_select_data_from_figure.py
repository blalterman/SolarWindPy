...
class TestSelectFromPlot2DProperties:
    ...
    def selector_setup(self):
        ...
        with patch('solarwindpy.plotting.select_data_from_figure.mpl.widgets.RectangleSelector'):
            plotter = Mock()
            plotter.data = pd.DataFrame({
                'x': np.random.randn(100),
                'y': np.random.randn(100),
...
class TestSelectFromPlot2DSampleData:
    ...
    def test_sample_data_with_other_selector(self, selector_setup):
        ...
        selector.sample_data(n=3, random_state=42, other_SelectFromPlot2D=other_selector)
    ...
    def test_sample_data_with_other_selector_list(self, selector_setup):
        ...
        selector.sample_data(n=3, random_state=42, other_SelectFromPlot2D=[other1, other2])
    ...
    def test_sample_data_insufficient_samples(self, selector_setup):
        ...
        with patch('solarwindpy.plotting.select_data_from_figure.logging.getLogger') as mock_get_logger:
            mock_logger = Mock()
            ...
            assert "Sample failed without replacement" in str(mock_logger.warning.call_args)
...
class TestSelectFromPlot2DEdgeCases:
    ...
    def test_sample_data_missing_already_selected(self, selector_setup):
        ...
        other_selector = Mock()
        other_selector.sampled_indices = pd.Index([200, 201, 202])  # Outside current data range
        ...
        with patch('solarwindpy.plotting.select_data_from_figure.logging.getLogger') as mock_get_logger:
            ...
            mock_logger.warning.assert_called()
            assert "None of `already_selected` found" in str(mock_logger.warning.call_args)
