...
class TestPlanningAgentsArchitecture:
    ...
    def test_checksum_management_format(self):
        ...
        for commit in actual_commits:
            assert re.match(r'^[a-f0-9]{7,}$', commit), f"Invalid commit hash format: {commit}"
    ...
    def test_acceptance_criteria_validation(self):
        ...
        for criteria in acceptance_criteria:
            ...
            assert re.search(pattern, content, re.IGNORECASE), f"Acceptance criteria not met: {criteria}"
    ...
    def test_time_estimation_tracking(self):
        ...
        assert re.search(time_pattern, content), "Should contain time estimates in min or h format"
...
class TestPlanningAgentsIntegration:
    ...
    def test_git_branch_operations(self):
        ...
        try:
            ...
            assert 'master' in result.stdout, "Should be able to list branches including master"
    ...
    def test_checksum_pattern_matching(self):
        ...
        for test_input, expected in test_patterns:
            ...
            assert is_valid == expected, f"Pattern matching failed for '{test_input}': expected {expected}, got {is_valid}"
...
def test_planning_agents_architecture_meta():
    ...
    assert "solarwindpy/tests" in str(current_file), "Test should be in solarwindpy/tests directory"
