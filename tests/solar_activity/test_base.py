...
from solarwindpy.solar_activity.base import (
    Base, ID, DataLoader, ActivityIndicator, IndicatorExtrema
)
...
class TestDataLoaderClass:
    ...
    def concrete_dataloader_class(self):
        ...
        class TestDataLoader(DataLoader):
            ...
            def download_data(self, new_data_path, old_data_path):
                ...
                sample_file = new_data_path / f"{pd.Timestamp('today').strftime('%Y%m%d')}.csv"
            ...
            def load_data(self):
                ...
                if fpath.exists():
                    ...
                    self._data = pd.DataFrame({
                        'test_value': np.random.uniform(0, 1, 10)
                    }, index=dates)
    ...
    def test_dataloader_get_data_ctime_no_files(self, concrete_dataloader_class, tmp_path):
    ...
    def test_dataloader_get_data_ctime_with_files(self, concrete_dataloader_class, tmp_path):
    ...
    def test_dataloader_maybe_update_stale_data(self, concrete_dataloader_class, tmp_path):
...
class TestActivityIndicatorClass:
    ...
    def test_activity_indicator_interpolate_data_with_nans(self, concrete_activity_indicator):
        ...
        source_dates = pd.date_range('2020-01-01', periods=5, freq='D')
        source_data = pd.DataFrame({
            'value': [1.0, np.nan, 3.0, 4.0, 5.0]  # Contains NaN
        }, index=source_dates)
...
class TestIndicatorExtremaClass:
    ...
    def test_indicator_extrema_initialization(self, concrete_indicator_extrema, synthetic_extrema_data):
    ...
    def test_indicator_extrema_calculate_intervals(self, concrete_indicator_extrema, synthetic_extrema_data):
        ...
        assert all(isinstance(interval, pd.Interval) or pd.isna(interval) 
                  for interval in intervals['Rise'].dropna())
    
    def test_indicator_extrema_cut_spec_by_interval(self, concrete_indicator_extrema, synthetic_extrema_data):
    ...
    def test_indicator_extrema_cut_spec_invalid_kind(self, concrete_indicator_extrema, synthetic_extrema_data):
    ...
    def test_indicator_extrema_calculate_extrema_bands(self, concrete_indicator_extrema, synthetic_extrema_data):
    ...
    def test_indicator_extrema_calculate_extrema_bands_dual_window(self, concrete_indicator_extrema, synthetic_extrema_data):
    ...
    def test_indicator_extrema_cut_about_extrema_bands(self, concrete_indicator_extrema, synthetic_extrema_data):
    ...
    def test_indicator_extrema_bands_error_before_calculation(self, concrete_indicator_extrema, synthetic_extrema_data):
        """Test that accessing extrema_bands before calculation raises error."""
        instance = concrete_indicator_extrema(synthetic_extrema_data)
        
        with pytest.raises(AttributeError, match="Have you called.*calculate_extrema_bands"):
