...
class TestSIDC:
    ...
    def test_initialization(self, mock_sidc_id_class, mock_ssn_extrema_class, mock_sidc_loader_class, 
                           mock_loader_data, mock_extrema_data):
    ...
    def test_run_normalization_max(self, mock_loader_data, mock_extrema_data):
        ...
        with patch.object(SIDC, '_init_logger'):
            ...
            cut_data = pd.Series([pd.Interval(0, 100)] * len(mock_loader_data), 
                                index=mock_loader_data.index, name='cycle')
    ...
    def test_interpolate_data(self, mock_loader_data):
        ...
        with patch.object(SIDC, '_init_logger'):
            ...
            expected_result = pd.Series(np.random.uniform(10, 100, len(target_index)), 
                                      index=target_index, name='ssn')
            
            with patch.object(SIDC.__bases__[0], 'interpolate_data', return_value=expected_result):
...
class TestSIDCEdgeCases:
    ...
    def test_normalized_property_without_nssn_column(self, mock_loader_data):
        ...
        with patch.object(SIDC, '_init_logger'):
            ...
            expected_normalized = pd.Series(np.random.uniform(0, 1, len(mock_loader_data)),
                                          index=mock_loader_data.index, name='nssn')
