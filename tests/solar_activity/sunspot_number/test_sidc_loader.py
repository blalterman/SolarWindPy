...
class TestSIDCLoaderCore:
    ...
    def test_data_path_property_structure(self):
        ...
        with patch.object(SIDCLoader.__bases__[0], 'data_path', new_callable=lambda: Path("/tmp/test")):
            with patch.object(SIDCLoader, '_init_logger'):
                loader = SIDCLoader("m", "http://example.com")
    ...
    def test_download_data_calls_read_csv(self, mock_read_csv, tmp_path):
        ...
        mock_csv = pd.DataFrame({
            'year': [1996], 'month': [1], 'year_fraction': [1996.042],
            'ssn': [17.5], 'std': [8.0], 'n_obs': [14], 'definitive': [True]
        })
    ...
    def test_download_data_invalid_key_raises_error(self, tmp_path):
        ...
        with pytest.raises(NotImplementedError, match="You have not yet used the SSN specified"):
            loader.download_data(new_data_path, old_data_path)
    ...
    def test_load_data_calls_parent_and_ssn_extrema(self, mock_ssn_extrema_class, mock_parent_load_data):
...
class TestSIDCLoaderEdgeCases:
    ...
    def test_path_creation_does_not_create_directories(self):
        ...
        with patch.object(SIDCLoader.__bases__[0], 'data_path', new_callable=lambda: Path("/nonexistent/test")):
            with patch.object(SIDCLoader, '_init_logger'):
                loader = SIDCLoader("m", "http://example.com")
