...
class TestSSNExtrema:
    ...
    def sample_ssn_extrema_csv(self):
        ...
        csv_content = """# Solar Cycle Extrema Data
...
25,2019-12-01,2025-07-01"""
    ...
    def test_load_or_set_data_empty_args(self, sample_ssn_extrema_csv):
        ...
        with patch('pandas.read_csv') as mock_read_csv, \
            ...
            stacked = pd.Series(['1964-10-01', '1968-11-01', '1976-03-01', '1979-12-01', 
                                '1986-09-01', '1989-07-01'])
            unstacked = pd.DataFrame({
                'Min': [pd.Timestamp('1964-10-01'), pd.Timestamp('1976-03-01'), pd.Timestamp('1986-09-01')],
                'Max': [pd.Timestamp('1968-11-01'), pd.Timestamp('1979-12-01'), pd.Timestamp('1989-07-01')]
            }, index=[20, 21, 22])
            unstacked.columns.names = ['kind']
            
            mock_read_csv.return_value = raw_data
            
            with patch.object(pd.DataFrame, 'stack', return_value=stacked), \
    ...
    def test_data_format_after_loading(self):
        ...
        with patch('pandas.read_csv') as mock_read_csv, \
            ...
            with patch('pandas.to_datetime') as mock_to_datetime:
                ...
                stacked_data = pd.Series(['2008-12-01', '2014-04-01', '2019-12-01', '2025-07-01'])
                
                # Mock to_datetime to return datetime series
                datetime_series = pd.Series([
                    pd.Timestamp('2008-12-01'), pd.Timestamp('2014-04-01'),
                    pd.Timestamp('2019-12-01'), pd.Timestamp('2025-07-01')
                ])
                mock_to_datetime.return_value = datetime_series
                
                with patch.object(pd.DataFrame, 'stack', return_value=stacked_data), \
