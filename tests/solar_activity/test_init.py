...
class TestGetAllIndices:
    ...
    def mock_lisird_data(self):
        ...
        lalpha_data = pd.DataFrame({
            'irradiance': np.random.uniform(0.001, 0.005, 100)
        }, index=dates)
        ...
        mgii_data = pd.DataFrame({
            'mg_index': np.random.uniform(0.25, 0.30, 100)
        }, index=dates)
    ...
    def test_get_all_indices_structure(self, dummy_lisird_class, dummy_sidc_class):
        ...
        with patch('solarwindpy.solar_activity.lisird.lisird.LISIRD', dummy_lisird_class):
            with patch('solarwindpy.solar_activity.sunspot_number.sidc.SIDC', dummy_sidc_class):
        ...
        assert list(result.columns.get_level_values(0).unique().sort_values()) == sorted(expected_columns)
    ...
    def test_get_all_indices_data_aggregation(self, dummy_lisird_class, dummy_sidc_class):
    ...
    def test_get_all_indices_mgii_date_conversion(self, dummy_lisird_class, dummy_sidc_class):
        ...
        mgii_data_with_time = pd.DataFrame({
            'mg_index': np.random.uniform(0.25, 0.30, 50)
        ...
        class DummyLISIRDWithTime:
            def __init__(self, key):
                ...
                if key == 'MgII':
                    ...
                    if key == 'Lalpha':
                        self._data = pd.DataFrame({'irradiance': np.random.uniform(0.001, 0.005, 50)}, index=dates)
                    else:  # CaK
                        self._data = pd.DataFrame({'irradiance': np.random.uniform(1000, 2000, 50), 'milliseconds': np.zeros(50)}, index=dates)
                
            @property
            def data(self):
                return self._data
        
        with patch('solarwindpy.solar_activity.lisird.lisird.LISIRD', DummyLISIRDWithTime):
            with patch('solarwindpy.solar_activity.sunspot_number.sidc.SIDC', dummy_sidc_class):
                result = sa.get_all_indices()
        
        # Verify that the result index only contains dates (no time component)
        assert all(timestamp.time() == pd.Timestamp('00:00:00').time() 
                  for timestamp in result.index[:5])  # Check first 5 entries
...
class TestRealWorldIntegration:
    ...
    def dummy_lisird_class(self):
        ...
        dates = pd.date_range('2020-01-01', periods=100, freq='D')
        mock_data = {
            'Lalpha': pd.DataFrame({'irradiance': np.random.uniform(0.001, 0.005, 100)}, index=dates),
            'CaK': pd.DataFrame({'irradiance': np.random.uniform(1000, 2000, 100), 'milliseconds': np.zeros(100)}, index=dates),
            'MgII': pd.DataFrame({'mg_index': np.random.uniform(0.25, 0.30, 100)}, index=dates)
        }
    ...
    def test_complete_get_all_indices_workflow(self, dummy_lisird_class, dummy_sidc_class):
        ...
        with patch('solarwindpy.solar_activity.lisird.lisird.LISIRD', dummy_lisird_class):
            with patch('solarwindpy.solar_activity.sunspot_number.sidc.SIDC', dummy_sidc_class):
                ...
                for expected_col in expected_top_level:
                    assert expected_col in actual_top_level, f"Missing column: {expected_col}"
