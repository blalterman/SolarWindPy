...
class TestColors:
    ...
    def test_color_methods(self):
        ...
        with patch('sys.stdout.isatty', return_value=True), \
             patch.dict(os.environ, {}, clear=True):
            assert statusline.Colors.red("test") == f"{statusline.Colors.RED}test{statusline.Colors.RESET}"
            assert statusline.Colors.yellow("test") == f"{statusline.Colors.YELLOW}test{statusline.Colors.RESET}"
            assert statusline.Colors.green("test") == f"{statusline.Colors.GREEN}test{statusline.Colors.RESET}"
...
class TestUsageIndicator:
    ...
    def test_usage_green_fresh(self):
        ...
        with patch('time.time', return_value=1000), \
             ...
             patch('pathlib.Path.read_text', return_value='999.5'):  # 0.5 hours ago
            with patch('sys.stdout.isatty', return_value=False):
...
if __name__ == "__main__":
    ...
    with patch('sys.stdout.isatty', return_value=True):
        ...
        with patch('subprocess.run') as mock_run, \
             ...
             patch('statusline.estimate_token_usage', return_value=statusline.Colors.yellow('160k')), \
             patch('statusline.get_compaction_indicator', return_value=statusline.Colors.yellow('●○○')), \
             patch('statusline.get_usage_indicator', return_value=statusline.Colors.green('█████')):
